                            ...
                            <div class="panel-heading" role="tab">
                                <div class="panel-title">
                                    <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                                        <div class="lgx-single-schedule"  
                                            :class="{
                                                'expired-event': moment(userTimezone(dateToFullDate(selected_dates[index][0]+' '+schedules[index].from_time, schedules[index].from_date),'dddd LL').format('dddd LL'),'dddd LL').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss'),
                                                'outofstock-event': !available_dates[index][0]
                                            }"
                                            @click="
                                                (!(moment(userTimezone(dateToFullDate(selected_dates[index][0]+' '+schedules[index].from_time, schedules[index].from_date),'dddd LL').format('dddd LL'),'dddd LL').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')) && available_dates[index][0])
                                                ? selectDates(
                                                userTimezone(dateToFullDate(selected_dates[index][0]+' '+schedules[index].from_time,schedules[index].from_date),'dddd LL').format('dddd LL'),
                                                 userTimezone(dateToFullDate(selected_dates[index][selected_dates[index].length - 1]+' '+schedules[index].from_time, schedules[index].from_date),'dddd LL').format('dddd LL'),
                                                changeTimeFormat(userTimezone(moment(schedules[index].from_date).format('YYYY-MM')+'-'+selected_dates[index][0]+' '+schedules[index].from_time, 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss')), 
                                                changeTimeFormat(userTimezone(moment(schedules[index].to_date).format('YYYY-MM')+'-'+selected_dates[index][0]+' '+schedules[index].to_time, 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss')) 
                                            ) : null" 
                                        >
                                            <div class="schedule-info pl-0"> 
                                                ...
                                                <!-- expired -->
                                                <h4 class="time event-ended" v-if="moment(userTimezone(dateToFullDate(selected_dates[index][0]+' '+schedules[index].from_time,schedules[index].from_date),'dddd LL').format('dddd LL'),'dddd LL').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')"
                                                >
                                                    <span>{{ trans('em.ended') }}</span>
                                                </h4>
                                                ...
                                                <!-- filling fast -->
                                                <h4 class="time event-outofstock mr-3" v-if="
                                                    !(moment(userTimezone(dateToFullDate(selected_dates[index][0]+' '+schedules[index].from_time,schedules[index].from_date),'dddd LL').format('dddd LL'),'dddd LL').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')) &&  positiveInteger(available_dates[index][0])"
                                                >
                                                    <span>{{ trans('em.filling_fast') }}</span>
                                                </h4>
    ...
    methods: {
        ...
        convert_to_local_tz(){
            ...
            this.schedules.forEach(function(value, key) {
                ...
                this.local_from_date[key] = this.convert_date_to_local(this.schedules[key].from_date+' '+this.schedules[key].from_time);
                this.local_to_date[key]   = this.convert_date_to_local(this.schedules[key].to_date+' '+this.schedules[key].to_time);
                this.local_from_time[key] = this.userTimezone(this.schedules[key].from_date+' '+this.schedules[key].from_time, "YYYY-MM-DD HH:mm:ss").format(date_format.vue_time_format);
                this.local_to_time[key]   = this.userTimezone(this.schedules[key].from_date+' '+this.schedules[key].to_time, "YYYY-MM-DD HH:mm:ss").format(date_format.vue_time_format);

            }.bind(this));
                
            // convert all dates to local timezone
            this.local_start_date   = this.convert_date_to_local(this.event.start_date+' '+this.event.start_time);
            this.local_end_date     = this.convert_date_to_local(this.event.end_date+' '+this.event.end_time);
        },
